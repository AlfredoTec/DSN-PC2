...
/**
 * GET /api/usuarios
 * Lista usuarios según el alcance del rol autenticado.
 * @param {import('express').Request} req - req.user con { id, roles, tienda_id }
 * @param {import('express').Response} res - 200 Usuario[] con roles agregados
 ...
 * @param {import('express').Request} req - Body: { email, password, nombre_completo, tienda_id? }
 * @param {import('express').Response} res - 201 { id, email, nombre_completo, tienda_id }
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
...
/**
 * PUT /api/usuarios/:id
 * Actualiza datos del usuario. Admin: total; Gerente: limitado a su tienda.
 * @param {import('express').Request} req - Params: { id }, Body: { nombre_completo?, activo?, tienda_id? }
 * @param {import('express').Response} res - 200 Usuario simplificado
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
 ...
 * @param {import('express').Request} req - Params: { id }, Body: { rolNombre }
 * @param {import('express').Response} res - 200 { message }
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
exports.assignRole = async (req, res, next) => {
  try {
    ...
    if (!actorId) return res.status(401).json({ message: 'Usuario autenticado inválido' });
    ...
    if (!actor) return res.status(401).json({ message: 'Usuario asignador no existe' });

    await prisma.usuarioRol.upsert({
      where: { usuario_id_rol_id: { usuario_id: user.id, rol_id: role.id } },
      update: {},
      create: { usuario_id: user.id, rol_id: role.id, asignado_por: actor.id }
    });

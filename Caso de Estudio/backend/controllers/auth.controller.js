 ...
 * @returns {Promise<string[]>} Arreglo con nombres de roles, p. ej. ['Admin', 'Gerente'].
 */
 ...
 * POST /api/auth/register
 * Registra un usuario nuevo con validación de contraseña y email único.
 * @param {import('express').Request} req - Body: { email: string, password: string, nombre_completo: string, tienda_id?: number }
 * @param {import('express').Response} res - 201 { id, email, nombre_completo, tienda_id, message }
...
/**
 * POST /api/auth/login
 * Inicia sesión con email y password. Si el usuario tiene MFA activa, retorna mfaToken temporal.
 * @param {import('express').Request} req - Body: { email: string, password: string }
 * @param {import('express').Response} res - 200 { mfaRequired: true, mfaToken } | { token, user }
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
...
/**
 * POST /api/auth/enable-mfa
 * Inicia la configuración de MFA TOTP. Acepta mfaToken temporal o JWT de sesión en Authorization.
 * @param {import('express').Request} req - Body: { mfaToken?: string } o Header: Authorization: Bearer <JWT>
 * @param {import('express').Response} res - 200 { otpauthUrl, otpauth, qrDataUrl }
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
...
/**
 * POST /api/auth/verify-mfa-setup
 * Verifica el primer código TOTP y habilita definitivamente la MFA.
 * @param {import('express').Request} req - Body: { mfaToken?: string, code: string }
...
/**
 * POST /api/auth/verify-mfa
 * Verifica el código TOTP durante el login MFA y retorna el JWT de sesión si es correcto.
 * @param {import('express').Request} req - Body: { mfaToken: string, code: string }
 * @param {import('express').Response} res - 200 { token, user }
 * @param {import('express').NextFunction} next
 * @returns {Promise<void>}
 */
